...
Xmatrix = zeros(n(2)*2, 3);
ZeroMatrix = zeros(n(2)*2, 3);
ZeroMatrix(:, 3) = 1;
j = 1;
for i=1:n(2)*2
    Xmatrix(i, 1) = j*j;
    Xmatrix(i, 2) = j;
    Xmatrix(i, 3) = 1;
    j = (mod(i,2)==0)*(j+1) + (mod(i,2)~=0)*j;
end
A = [1:1:101*2; 1:1:101*2; 1:1:101*2]';
A = (mod(A,2)==0).*ZeroMatrix + (mod(A,2)~=0).*Xmatrix;
b = [1:1:101*2]';
j = 1;
for i=1:n(2)*2
    b(i) = (mod(i, 2)==0).*0 + (mod(i, 2)~=0).*YOld(j);
    j = (mod(i,2)==0)*(j+1) + (mod(i,2)~=0)*j;
end
B = (inv(A'*A)*A');
cx = B*b;
...
maxInter = 30;
plot(X2(1:maxInter), YfittedPos1(1:maxInter), 'color','r'); hold on;
plot(X2(1:maxInter), YfittedPos2(1:maxInter), 'color','b');
plot(X2(1:maxInter), YOld(1:maxInter), 'color','y'); 
plot(X2(1:maxInter), YfittedE(1:maxInter), 'color','g');
plot(X2(1:maxInter), MyYfitted(1:maxInter), 'color','black');

maxInter = 10;
plot(X2, YOld, 'color','r'); hold on;
plot(X2, Yfitted2, 'color','b');
maxInter = 30;
plot(X2(1:maxInter), Yfitted3(1:maxInter), 'color','y');hold on;
plot(X2(1:maxInter), YOld(1:maxInter), 'color','r'); 
plot(X2(1:maxInter), YfittedE(1:maxInter), 'color','b');
